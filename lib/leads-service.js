...
/**
 * 💰 Estimate potential value: the business's own average job value
 * (customers.avg_job_value, set in Settings) scaled by temperature.
 * Returns null when the business hasn't set an average job value —
 * better no number than a made-up one. (The old version hardcoded
 * BizzyBot's own legacy subscription prices as every lead's value.)
 */
export function estimateLeadValue(lead, avgJobValue, temperature) {
  const base = Number(avgJobValue);
...
export async function updateLeadScoring(customerId, contactId) {
  try {
    ...
    // The business's own average job value drives potential_value
    const avgRes = await query(
      `SELECT avg_job_value FROM customers WHERE id = $1 LIMIT 1`,
      [customerId]
    ).catch(() => ({ rows: [] }));
    const avgJobValue = avgRes.rows[0]?.avg_job_value;

    // Calculate new score
    const score = calculateLeadScore(lead);
    const temperature = classifyLeadTemperature(score, lead);
    const value = estimateLeadValue(lead, avgJobValue, temperature);
...
/**
 * 📶 Bump an engagement counter on a contact (found by email/phone) and
 * rescore — WITHOUT inserting an ai_analytics_events row. Used for signals
 * the behavior analyzer already logs to analytics (phone_requested) or that
 * only matter for scoring (pricing_discussed), so analytics never
 * double-counts.
 */
export async function recordEngagementSignal(customerId, { email, phone }, eventType) {
  try {
    const counters = {
      'phone_requested': 'phone_request_count = COALESCE(phone_request_count, 0) + 1',
      'pricing_discussed': 'pricing_discussion_count = COALESCE(pricing_discussion_count, 0) + 1',
      'appointment_booked': 'appointment_count = COALESCE(appointment_count, 0) + 1',
    };
    const counterSql = counters[eventType];
    if (!counterSql || (!email && !phone)) return { success: false };

    await query(`ALTER TABLE contacts ADD COLUMN IF NOT EXISTS pricing_discussion_count INTEGER DEFAULT 0`).catch(() => {});

    const found = await query(
      `SELECT id FROM contacts
       WHERE customer_id = $1 AND ((email = $2 AND $2 IS NOT NULL) OR (phone = $3 AND $3 IS NOT NULL))
       ORDER BY last_interaction_at DESC NULLS LAST LIMIT 1`,
      [customerId, email || null, phone || null]
    );
    const contactId = found.rows[0]?.id;
    if (!contactId) return { success: false };

    await query(
      `UPDATE contacts SET ${counterSql}, updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND customer_id = $2`,
      [contactId, customerId]
    );
    await updateLeadScoring(customerId, contactId);
    return { success: true, contactId };
  } catch (error) {
    console.error('❌ recordEngagementSignal error:', error.message);
    return { success: false, error: error.message };
  }
}
...
export async function trackLeadEventWithContact(customerId, contactId, eventData) {
  try {
    ...
    // Update contact metrics. Both event-name generations are mapped —
    // real bookings emit 'appointment_booked' and the behavior analyzer
    // emits 'phone_requested'; the *_scheduled/_request names are legacy.
    const updateMetrics = {
