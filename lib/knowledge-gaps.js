...
export function buildTranscriptScanPrompt(businessName) {
  ...
  return `You review a phone call transcript between a caller and ${name}'s AI assistant. ` +
    ...
    `Choosing the topic: anything about discounts, rates, fees, quotes or what something costs is "pricing"; ` +
    `where you travel to or whether you cover somewhere is "service_area"; when you are open is "hours"; ` +
    `booking, availability or appointments is "scheduling"; whether you do a particular job is "services"; ` +
    `how the work runs, timelines or what to expect is "process"; guarantees or callbacks on completed work is "warranty"; ` +
    `deposits, methods or financing is "payment". Use "other" ONLY when it genuinely fits none of these. ` +
    `question = the UNDERLYING question the caller wanted answered, written plainly and under 200 characters — ` +
    `not their exact wording. Strip conversational scaffolding: "is there any way to find out if you guys could maybe ` +
    `do a discount for veterans" is the question "Do you offer a veteran discount?". Two callers asking the same ` +
    `thing in different words must produce the same question and the same topic. ` +
    `caller_name = the caller's first name if they gave one, otherwise null. Never write "Unknown", "N/A" or similar — use null. ` +
