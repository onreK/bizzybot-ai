...
export async function sendHotLeadAlert(clerkUserId, { contactName, contactEmail, contactPhone, channel, message, score }) {
  try {
    ...
    const html = `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;margin:0 auto;background:#0D1117;border-radius:12px;overflow:hidden;border:1px solid #30363D;">
        <div style="background:linear-gradient(135deg,#7c3aed,#6d28d9);padding:24px 28px;">
          ...
          <p style="margin:0;color:rgba(255,255,255,0.75);font-size:13px;">${customer.business_name || 'Your BizzyBot'} &nbsp;·&nbsp; via ${channelLabel}</p>
        </div>
        <div style="padding:24px 28px;background:#161B22;">
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <tr><td style="color:#8B949E;font-size:12px;padding:5px 0;width:72px;vertical-align:top;">From</td><td style="color:#E6EDF3;font-size:15px;font-weight:600;padding:5px 0;">${contactDisplay}</td></tr>
            ${contactEmail ? `<tr><td style="color:#8B949E;font-size:12px;padding:5px 0;">Email</td><td style="color:#E6EDF3;font-size:14px;padding:5px 0;">${contactEmail}</td></tr>` : ''}
            ${contactPhone ? `<tr><td style="color:#8B949E;font-size:12px;padding:5px 0;">Phone</td><td style="color:#E6EDF3;font-size:14px;padding:5px 0;">${contactPhone}</td></tr>` : ''}
            <tr><td style="color:#8B949E;font-size:12px;padding:5px 0;">Channel</td><td style="color:#E6EDF3;font-size:14px;padding:5px 0;">${channelLabel}</td></tr>
          ...
          <div style="background:#0D1117;border:1px solid #30363D;border-radius:8px;padding:14px;margin-bottom:20px;">
            ...
            <p style="margin:0;color:#C9D1D9;font-size:13px;line-height:1.6;">${String(message).slice(0, 350).replace(/</g, '&lt;').replace(/>/g, '&gt;')}${message.length > 350 ? '…' : ''}</p>
          </div>` : ''}
    ...
    await resend.emails.send({
      ...
      subject: `🔥 Hot Lead — ${contactDisplay} (${channelLabel})`,
      html,
    });
