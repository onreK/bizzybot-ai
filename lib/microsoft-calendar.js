...
// Returns up to 6 free 1-hour slots across the next 5 business days (9am–5pm UTC)
export async function getAvailableSlots(clerkUserId) {
  ...
  try {
    ...
    while (available.length < 6 && cursor < end) {
      ...
      // Skip weekends, skip outside 9am–4pm UTC (so events end by 5pm)
    ...
    return { slots: available, needsReconnect: false };
  } catch (err) {
...
// Format slots as readable text for the AI system prompt
export function formatSlotsForAI(slots) {
  ...
  const lines = slots.map(s => {
    ...
    const isoStart = d.toISOString().slice(0, 16); // YYYY-MM-DDTHH:MM
...
export async function createCalendarEvent(clerkUserId, {
  ...
  const event = {
    subject: `Meeting with ${attendeeName || attendeeEmail || 'Lead'} — ${businessName || 'BizzyBot AI'}`,
    start:   { dateTime: start.toISOString(), timeZone: 'UTC' },
    end:     { dateTime: end.toISOString(),   timeZone: 'UTC' },
    body: {
      contentType: 'Text',
      content: `This appointment was scheduled automatically via BizzyBot AI on behalf of ${businessName || 'your business'}.`,
    },
    ...(attendeeEmail ? {
      attendees: [{
