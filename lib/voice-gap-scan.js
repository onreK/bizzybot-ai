...
export async function processVoiceGapScan({ customerId, vapiCallId, callerPhone, transcript, existingContact }) {
  try {
    ...
    // Fill in the caller's name if they gave one. This runs BEFORE the
    // no-gaps return: a caller who introduces themselves and gets every
    // question answered is exactly the case that was leaving leads named
    // "Unknown" while the call summary beside them said "The user, Zach
    // Gordon, asked about...". The transcript scan already extracted it;
    // nothing was carrying it back to the contact record.
    //
    // Only fills an EMPTY name — a name typed by the owner, or captured with
    // more care elsewhere, is never overwritten by a transcript's guess.
    if (callerName && callerPhone) {
      await query(
        ...
        [callerName, customerId, callerPhone]
      )
        .then((r) => {
          if (r.rowCount > 0) console.log(`🧠 [GAPS] named contact ${callerPhone} as "${callerName}"`);
        })
        .catch((err) => console.error('⚠️ [GAPS] could not set contact name:', err.message));
    }
