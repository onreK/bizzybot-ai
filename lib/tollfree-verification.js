...
export async function submitTollfreeVerification({ clerkUserId, phoneNumberSid }) {
  ...
  // Business type + registration params. Sole proprietors need no EIN;
  // everyone else supplies an EIN (US authority/country inferred).
  const businessParams = { businessType };
  if (!isSoleProprietor) {
    businessParams.businessRegistrationNumber = registrationNumber;
    businessParams.businessRegistrationAuthority = 'EIN';
    businessParams.businessRegistrationCountry = 'US';
  }
