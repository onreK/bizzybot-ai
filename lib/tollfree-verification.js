...
export async function ensureVerificationInfoTable() {
  ...
  await query(`ALTER TABLE sms_verification_info ADD COLUMN IF NOT EXISTS business_email TEXT`).catch(() => {});
 ...
 */
export async function submitTollfreeVerification({ clerkUserId, phoneNumberSid, dryRun = false }) {
  if (!twilioClient && !dryRun) throw new Error('Twilio not configured');

  await ensureTfvColumns();

  // A rejected verification must be EDITED in place within Twilio's edit
  // window (keeps the priority review spot); creating a fresh one is only
  ...
  // Digits of the toll-free number — used for the per-customer branded
  // evidence pages (/sms-optin/{digits}, /sms-terms/{digits}).
  const phoneDigits = String(existing.phone_number || '').replace(/\D/g, '').replace(/^1(?=\d{10}$)/, '');
  const editSid = existing.tfv_status === 'TWILIO_REJECTED' && existing.tfv_sid
    ? existing.tfv_sid
    : null;
  ...
  // Contact person MUST be a real person's full name — Twilio rejects
  // generic contacts outright (error 30483; our old "Bizzy Team"-style
  // fallback caused exactly that), so missing names block submission.
  const contactFirstName = (info.contact_first_name || '').trim();
  const contactLastName = (info.contact_last_name || '').trim();

  // Business email must be on the business's domain, or a free address
  // that's listed on the business website (error 30482). No fallback:
  // a guessed email (signup/admin) is a guaranteed rejection, so a
  // missing one blocks submission until the customer provides it.
  const businessEmail = (info.business_email || '').trim();

  // Legal name for carrier records (error 30484); brand stays what the AI
  // uses with leads. Back-compat: customers from before this field treat
  // their saved business name as the legal name until they edit onboarding.
  ...
  const needsInfo = [];
  if (!legalName) needsInfo.push('legalBusinessName');
  ...
  if (!contactFirstName || !contactLastName) needsInfo.push('contactName');
  if (!businessEmail) needsInfo.push('businessEmail');
  ...
  // Carriers match this field against official records; the DBA form ties
  // the registered legal entity to the brand consumers actually see.
  const businessName = brandName.toLowerCase() !== legalName.toLowerCase()
    ? `${legalName} DBA ${brandName}`
    : legalName;

  // Per-customer branded evidence pages; fall back to the legacy generic
  // pages if this row's phone number is malformed (never submit dead links).
  ...
  const data = {
    ...
    UseCaseSummary:
      `${brandName} uses this number to reply to inbound SMS inquiries from its own customers and leads. ` +
      `Consumers initiate contact by texting the number, which ${brandName} advertises on its website and business listings. ` +
    ...
    ProductionMessageSample:
      `Hi! Thanks for reaching out to ${brandName}. Happy to help — what service are you looking for? ` +
    ...
    AdditionalInformation:
      ...
      `Opt-in documentation: ${optInUrl}. SMS terms: ${termsUrl}`,
  };
  ...
  // Editing an existing verification: the number binding is create-only.
  if (editSid) delete data.TollfreePhoneNumberSid;

  // Dry run: show exactly what would be sent (and where), without submitting.
  if (dryRun) {
    console.log(`🧪 TFV DRY RUN for ${clerkUserId} (${editSid ? `edit ${editSid}` : 'new submission'}):`, JSON.stringify(data, null, 2));
    return { submitted: false, dryRun: true, editSid: editSid || null, payload: data };
  }

  const response = await twilioClient.request({
    method: 'POST',
    uri: editSid
      ? `https://messaging.twilio.com/v1/Tollfree/Verifications/${editSid}`
  ...
  await query(
    `UPDATE customer_phone_numbers
     SET tfv_sid = $1, tfv_status = $2, tfv_rejection_reason = NULL,
         tfv_submitted_at = NOW(), updated_at = NOW()
     WHERE twilio_sid = $3`,
    [verification.sid, verification.status || 'PENDING_REVIEW', phoneNumberSid]
  ).catch(() => {});

  console.log(`✅ TFV ${editSid ? 're' : ''}submitted for ${businessName}: ${verification.sid} (${verification.status})`);
...
export async function checkPendingTollfreeVerifications() {
  ...
  const pending = await query(
    `SELECT cpn.tfv_sid, cpn.tfv_status, cpn.phone_number, cpn.twilio_sid, cpn.clerk_user_id,
  ...
  for (const row of pending.rows) {
    try {
      ...
      if (verification.status === 'TWILIO_APPROVED') {
        ...
        // Already recorded and admin already emailed — don't repeat hourly.
        if (row.tfv_status === 'TWILIO_REJECTED') continue;
...
async function sendVerificationApprovedEmail(row) {
  try {
    ...
    const sendResult = await sendEmail({
      ...
      `,
    }, `TFV approved email (${row.phone_number})`);
    if (sendResult.sent) console.log(`✅ Verification-approved email sent to ${row.email}`);
  } catch (err) {
...
async function sendVerificationRejectedAdminEmail(row, reason) {
  try {
    await sendEmail({
      ...
      `,
    }, `TFV rejected admin email (${row.phone_number})`);
  } catch (err) {
