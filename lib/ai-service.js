...
import { hasActiveAccess } from './trial-access.js';
...
export async function generateAIResponse({
  ...
  try {
    ...
    // Step 1.3: Trial fully ended, no subscription — go silent (no auto-reply on
    // any channel). Different from the softer usage-cap case below: this is an
    // account that never converted, not an active customer who hit a monthly cap.
    if (effectiveClerkUserId) {
      ...
      if (customer && !hasActiveAccess(customer)) {
        console.log(`🚫 [AI-SERVICE] Trial ended, no subscription — staying silent for ${effectiveClerkUserId}`);
        return {
          success: false,
          response: null,
    ...
    if (effectiveClerkUserId && customerConfig?.booking_auto_send !== false) {
      try {
        ...
        if (calResult.slots?.length > 0) {
          calendarContext = {
            slotsText: formatSlotsForAI(calResult.slots, calResult.timezone),
    ...
    if (bookingMatch && calendarContext && effectiveClerkUserId) {
      const startISO = bookingMatch[1]; // e.g. "2026-06-05T14:00" (UTC, from a [SLOT:] marker)

      // Only book times we actually offered — the AI can otherwise emit a
      // hallucinated or already-busy datetime and we'd book it blindly
      const isOfferedSlot = calendarContext.slots?.some(
        s => s.start.toISOString().slice(0, 16) === startISO
      );

      if (!isOfferedSlot) {
        console.warn(`⚠️ [BOOK:] time ${startISO} is not an offered slot — not booking`);
        const fallbackLine = customerConfig?.booking_url
          ? `\n\nIt looks like that exact time may no longer be available — you can grab a time that works here: ${customerConfig.booking_url}`
          : `\n\nLet me double-check that time is still open and confirm with you right away.`;
        formattedResponse = formattedResponse.replace(bookingMatch[0], fallbackLine);
      } else {
      try {
        ...
        await createCalendarEvent(effectiveClerkUserId, {
          ...
          attendeePhone: contactPhone || null,
        ...
        const friendlyTime = new Date(startISO + ':00Z').toLocaleString('en-US', {
          ...
          timeZone: calendarContext.timezone || 'America/New_York', timeZoneName: 'short',
        });
        // Book first, then ask for email (founder decision 2026-07-10): never
        // add friction before the booking — worst case we have their phone.
        const confirmation = contactEmail
          ? `A calendar invitation has been sent to ${contactEmail}.`
          : 'What\'s the best email for you, in case we need to send any details?';
        ...
        // Real booking — record it linked to the CONTACT so it drives lead
        // scoring (appointment_count → instant 'hot'), not just analytics.
        // Falls back to a bare analytics event when the lead is anonymous.
        if (customerConfig?.id) {
          try {
            let contactId = null;
            if (contactEmail || contactPhone) {
              const contactResult = await createOrUpdateContact(customerConfig.id, {
                email: contactEmail || null,
                phone: contactPhone || null,
                name: leadContext?.name || null,
                source_channel: channel,
              });
              contactId = contactResult.success ? contactResult.contact.id : null;
            }
            if (contactId) {
              await trackLeadEventWithContact(customerConfig.id, contactId, {
                type: 'appointment_booked',
                channel,
                message: `Booked ${startISO}`,
                metadata: { start: startISO, attendee: contactEmail || null },
              ...
              await query(`
                ...
                VALUES ($1, 'appointment_booked', $2, $3, 1.0, CURRENT_TIMESTAMP)
              `, [
                customerConfig.id,
                JSON.stringify({ start: startISO, attendee: contactEmail || null }),
                channel
              ]);
            }
          } catch (err) {
            console.error('⚠️ appointment_booked tracking failed:', err.message);
          }
        }
      } catch (bookErr) {
        console.error('⚠️ Calendar booking failed:', bookErr.message);
        // Don't leave the AI's "you're booked!" text standing when nothing
        // was booked — correct the claim honestly
        const failLine = customerConfig?.booking_url
          ? `\n\nActually — I hit a snag confirming that on the calendar. To be safe, please grab your time directly here: ${customerConfig.booking_url}`
          : `\n\nActually — I hit a snag confirming that on the calendar. The team will follow up shortly to lock in your time.`;
        formattedResponse = formattedResponse.replace(bookingMatch[0], failLine);
      }
      }
    }
    
    // Step 5.7: Track real document sends (the reply actually contains a
    // configured document link — not regex guessing)
    if (customerConfig?.id && Array.isArray(customerConfig.documents)) {
      const sentDocs = customerConfig.documents.filter(
        d => d?.link?.trim() && formattedResponse.includes(d.link.trim())
      );
      if (sentDocs.length > 0) {
        await query(`
          INSERT INTO ai_analytics_events
          (customer_id, event_type, metadata, channel, confidence_score, created_at)
          VALUES ($1, 'document_sent', $2, $3, 1.0, CURRENT_TIMESTAMP)
        `, [
          customerConfig.id,
          JSON.stringify({ documents: sentDocs.map(d => d.description || d.link) }),
          channel
        ]).catch(err => console.error('⚠️ document_sent event insert failed:', err.message));
      }
    }
    ...
    if (customerConfig && customerConfig.id) {
      ...
      // Feed engagement counters on the contact so lead scoring reflects
      // real buying signals (the analyzer logs analytics; these bump the
      // contact + rescore without double-logging).
      if (contactEmail || contactPhone) {
        const identity = { email: contactEmail, phone: contactPhone };
        if (trackedEvents.some(e => (e.type || e.event_type) === 'phone_requested')) {
          await recordEngagementSignal(customerConfig.id, identity, 'phone_requested').catch(() => {});
        }
        if (/\b(price|pricing|cost|costs|how much|quote|estimate|rates?)\b/i.test(userMessage || '')) {
          await recordEngagementSignal(customerConfig.id, identity, 'pricing_discussed').catch(() => {});
        }
      }
    }
    ...
    // throw, so awaiting it can never break the reply, and up to five quick
    // local queries (three CREATE ... IF NOT EXISTS, the dedup SELECT, the
    // INSERT) are cheaper than losing entries to a dropped promise. The extra
    // .catch below is a belt-and-braces guard: it must stay impossible for
    // this call to break a good reply even if a future edit to recordGap
    // introduces a throw.
    if (knowledgeGap && customerConfig?.id) {
      await recordGap({
        ...
        channel,
        contactEmail,
        contactPhone: contactPhone || channelSpecificData?.phoneNumber || null,
...
async function getCustomerAIConfiguration(customerEmail, clerkUserId, channel = 'email') {
  try {
    ...
    // Fall back to the email lookup whenever the id didn't resolve a customer.
    // Legacy Gmail connections stored a bad user_id (e.g. 'anonymous'), which
    // otherwise left the config empty and produced generic/placeholder replies.
    if (!customer && customerEmail) {
      const result = await query(
        'SELECT * FROM customers WHERE email = $1 ORDER BY id ASC LIMIT 1',
        [customerEmail]
      ).catch(() => ({ rows: [] }));
    ...
    if (!customer) {
      console.log('⚠️ [AI-SERVICE] No customer found (clerkUserId + email both missed)');
      return null;
    }
    ...
    const config = {
      ...
      // ⚠️ MUST BE LAST. ai_configs and email_settings each have their OWN `id`
      // primary key, so spreading them above silently overwrote customer.id —
      // e.g. customer 863 came out as id 4 (their email_settings row's id).
      // Every consumer of customerConfig.id treats it as a CUSTOMER id: lead
      // context lookups, contact creation, lead scoring, engagement signals,
      // document sends and the unanswered-questions queue. Pin it explicitly.
      id: customer.id,
...
function buildChannelSpecificPrompt(channel, customerConfig, channelData = {}, leadContext = null) {
  ...
  if (customerConfig?.has_calendar && customerConfig?.calendar_slots_text) {
    basePrompt += `\n\nCALENDAR: You can book appointments directly on the owner's calendar. Available slots (shown in the business's local time):\n${customerConfig.calendar_slots_text}\n\nWhen scheduling comes up: first present 2-3 of these available times (say them exactly as listed, including the timezone) and ask which works best. Once the lead confirms a specific slot, include the marker [BOOK:YYYY-MM-DDTHH:MM] exactly once in your reply, copying the datetime EXACTLY from that slot's [SLOT:...] marker — never invent or adjust it. Example: "Perfect, I've got you booked for Thursday at 2pm! [BOOK:2026-06-05T18:00]". Only use [BOOK:...] after the lead has explicitly confirmed a specific time, and only for times in the list above.${bookingUrl ? ` If none of the listed times work, offer the booking link as an alternative: ${bookingUrl}` : ''}`;
  }
...
function formatResponseForChannel(aiResponse, channel, customerConfig) {
  ...
  switch (channel) {
    case 'sms': {
      // SMS is NOT capped at 160 chars — Twilio auto-splits a longer reply into
      // segments and the recipient sees one whole message. The old 150-char cut
      // chopped pricing off mid-word ("100 voic..."). Only guard against a
      // runaway-length reply, and trim at a WORD boundary if we ever have to.
      const SMS_MAX = 640; // ~4 SMS segments — comfortably fits a full pricing reply
      if (response.length > SMS_MAX) {
        const slice = response.slice(0, SMS_MAX);
        const lastSpace = slice.lastIndexOf(' ');
        const trimmed = (lastSpace > 300 ? slice.slice(0, lastSpace) : slice).trimEnd();
        return trimmed + ' … reply or call for the full details.';
      }
      return response;
    }
...
export async function getLeadContextForResponse(customerId, identifier) {
  try {
    ...
    // Voice memory: pull this lead's recent AI phone calls (by phone number)
    // so text/email/chat replies can pick up where a call left off.
    const leadPhone = leadResult.lead.phone;
    if (leadPhone) {
      try {
        const digits = String(leadPhone).replace(/\D/g, '').slice(-10);
        if (digits.length === 10) {
          const calls = await query(
            `SELECT summary, transcript, COALESCE(started_at, created_at) AS called_at
             FROM vapi_call_logs
             WHERE customer_id = $1
               AND RIGHT(REGEXP_REPLACE(COALESCE(caller_phone, ''), '\\D', '', 'g'), 10) = $2
             ORDER BY COALESCE(started_at, created_at) DESC
             LIMIT 2`,
            [customerId, digits]
          );
          context.recentCalls = calls.rows
            .map(c => ({
              date: c.called_at ? new Date(c.called_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '',
              summary: (c.summary || '').trim() || (c.transcript || '').trim().slice(0, 300),
            }))
            .filter(c => c.summary);
        }
      } catch (callErr) {
        console.error('⚠️ Voice memory lookup failed:', callErr.message);
      }
    }
...
export function enrichPromptWithLeadContext(basePrompt, leadContext) {
  ...
  // Voice memory: summaries of this lead's recent phone calls with the AI
  if (leadContext.recentCalls?.length) {
    enrichedPrompt += '\n📞 PREVIOUS PHONE CALLS WITH THIS LEAD:\n';
    for (const call of leadContext.recentCalls) {
      enrichedPrompt += `- [${call.date}] ${call.summary}\n`;
    }
    enrichedPrompt +=
      'Use these to continue the conversation naturally (e.g. reference what they asked about on the call). ' +
      'Call transcripts are automated and may contain errors — never quote them verbatim or insist on details; ' +
      'if something seems off, ask the lead to confirm rather than assert.\n';
  }
