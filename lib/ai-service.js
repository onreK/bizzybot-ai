  ...
  try {
    ...
    if (bookingMatch && calendarContext && effectiveClerkUserId) {
      ...
      try {
        ...
        // Real booking — record it so Appointments on Analytics/Overview counts
        // actual calendar events, not regex-detected "scheduling mentioned"
        if (customerConfig?.id) {
          ...
          `, [
            ...
            channel
          ]).catch(err => console.error('⚠️ appointment_booked event insert failed:', err.message));
        }
      } catch (bookErr) {
