...
// GET: read the caller's saved verification info
...
export async function POST(request) {
  try {
    ...
    if (!(contactFirstName || '').trim() || !(contactLastName || '').trim()) {
      return NextResponse.json({ success: false, error: 'Authorized contact first and last name are required' }, { status: 400 });
    }
    const cleanBusinessEmail = (businessEmail || '').trim();
    if (cleanBusinessEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanBusinessEmail)) {
      return NextResponse.json({ success: false, error: 'Business email is not a valid email address' }, { status: 400 });
    }
