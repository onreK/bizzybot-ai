...
- [~] **3. Vapi voice end-to-end test** — DONE 2026-07-06: voice AI provisioned + Active on (866) 944-5685 (green status on /voice). Fixed en route: fragile customer JOIN (clerk_user_id-only) in vapi/provision, and invalid 11labs voice "rachel" → switched to Vapi-native "Elliot" (configurable via VAPI_VOICE_* env). Live call test (AI answers + transcript/minutes logged) pending user dialing the number.
