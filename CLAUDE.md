...
- **Vapi call recordings live in a PRIVATE bucket.** The `recordingUrl` on the end-of-call report is a Cloudflare R2 URL that returns `400 InvalidArgument/Authorization` to everyone, including an `<audio>` tag. The playable form is `artifact.presignedMonoUrl` from `GET https://api.vapi.ai/call/{id}` — but it carries `X-Amz-Expires=1800`, so it dies after **30 minutes**. NEVER store the presigned URL: it works on the day you ship and is broken for every recording older than half an hour. Store the raw URL as the marker a recording exists; `/api/vapi/recording/[callId]` mints a fresh signed one per play, checks the call belongs to the caller's customer, and 302-redirects so R2 serves the bytes.
- **Windows/ESM:** absolute-path imports in a scratch `.mjs` need a `file:///C:/...` URL; a bare `C:/...` throws `ERR_UNSUPPORTED_ESM_URL_SCHEME`.
- **Querying prod as the founder tests:** `railway variables --service Postgres --kv | grep DATABASE_PUBLIC_URL` then `DATABASE_URL="..." node script.mjs`, importing `lib/database.js`. Fastest way to verify a live test actually did what the UI claims — this caught four real bugs on 07-27.
...
- **Call recordings — SHIPPED + LIVE-VERIFIED 2026-07-28.** Every call announces recording in the greeting (hard-coded in `buildFirstMessage`, `lib/vapi.js` — not a customer setting, same reasoning as the SMS footer) and every recording is kept. Playback on `/voice` via `/api/vapi/recording/[callId]`, which is auth-gated and ownership-checked. See Code Gotchas for why a direct link cannot work.
- **Caller names from voice calls — SHIPPED 2026-07-28.** The transcript scan already extracted the caller's name but only wrote it to `knowledge_gaps`; leads stayed "Unknown" while the summary beside them named the person. Now written back to the contact, before the no-gaps early return, and only into an EMPTY name so an owner-typed name is never overwritten.
- **Toll-free SMS compliance footer — SHIPPED 2026-07-27.** Business name + "Reply STOP to opt out, HELP for help." on the first message of each conversation, every 10th after, and always on business-initiated follow-ups. Hard-coded in `lib/sms-compliance.js`, applied in the send path — not a setting and not a prompt instruction, because BizzyBot holds the toll-free verification and would absorb a customer removing it. Twilio's TFV guide requires business name + opt-out "at regular intervals"; live traffic previously carried neither while the approved sample promised it.
- **Landing page motion + chat widget — SHIPPED 2026-07-27.** Scroll reveals on all seven sections (`components/marketing/Reveal.js`, fails VISIBLE — content is not hidden in the server-rendered HTML) plus a looping hero dashboard that starts on scroll-into-view. BizzyBot's own chat widget on the landing page and every marketing page via `components/marketing/DemoWidget.js`.
...
**📌 SCOPED 07-27, NOT STARTED — public booking page** (`docs/superpowers/specs/2026-07-27-public-booking-page-scope.md`). A hosted per-customer page (`bizzybotai.com/book/acme-plumbing`) whose link goes in their Google Business Profile **Appointment link** field. Mostly assembly — Outlook availability, meeting length, timezone and event creation with invites all already exist and are used by the AI's `[BOOK:]` path. Key points: it is deliberately NOT Reserve with Google (that needs Google partner certification + Maps Booking API, premature at this merchant count); the biggest new risk is an unauthenticated endpoint writing real events to a real calendar, so rate limiting is mandatory; needs a `middleware.js` publicRoutes entry or crawlers get bounced to sign-in. **Build roadmap #2 (service-area + hours fields) FIRST** — it gives the page a real "do you cover my zip?" check like ServiceTitan's first step, for free.

**✅ VERIFIED 2026-07-28 — "KEEP YOUR EXISTING NUMBER" WORKS. Say this on sales calls.** A customer does NOT have to give out their BizzyBot number. They set their existing business line to forward to it (carrier-side, on their provider) and BizzyBot answers. **Live-tested: caller ID passes through the forward intact** — the call recorded the number the caller dialled *from*, not the forwarding line, so lead capture is correct. This removes the single biggest objection from every established business (numbers on trucks, signs, Google listings, years of reviews), which is exactly the segment with the most missed calls and the most money.

Two things to get right when selling it:
- **They must be on "AI answers first."** Their own phone system already did the try-a-human step; leaving them on "Ring my phone first" stacks two waits (~20s + ~18s) and the caller hangs up.
- **Voice only.** Their landline cannot text, so SMS still needs a BizzyBot number until Hosted SMS ships.

⚠️ **PRODUCT GAP: nothing in the UI tells them this is possible.** The capability works; it is undiscoverable. Scoped but NOT built: a "Keep your existing number" panel on `/voice` showing their BizzyBot number to forward to, the carrier codes, and an automatic switch to AI-answers-first. ~1 hour. Deliberately deferred — it is a scaling tool, not a selling tool, and white-glove onboarding covers the first ten customers. Build it around customer 20.

Testing note if this comes up again: iPhone's **Settings → Phone → Call Forwarding** is the quick path (unconditional forwarding answers the caller-ID question just as well as conditional). T-Mobile USSD codes need the DOUBLE asterisk — `**61*1XXXXXXXXXX#` — and `*61*` alone just drops the call. Business lines (VoIP/RingCentral/Ooma/Comcast Business) expose conditional forwarding as a web-portal checkbox, so consumer-mobile quirks are not representative of real customers.

**💡 LEAD-QUALIFICATION FILTER (from the same research):** ServiceTitan costs **$245–500/tech/month plus $5K–50K setup**. A business already running it is NOT a BizzyBot prospect — treat a ServiceTitan booking link in their Google listing as a disqualifier when building target lists. The buyer is the business beside them with no booking button and missed calls.

**📌 DESIGNED 07-27, DEFERRED UNTIL MORE CUSTOMERS — BizzyBot support chat in the customer dashboard.** Founder: "note it and we will build it in the future once we have more customers." Full design in memory ([[customer-support-widget]]). Summary so a future session doesn't re-derive it:
- Widget in the customer dashboard where BizzyBot answers ITS OWN customers' how-to questions. Distinct from the Web Chat page's live-test chat, which talks to the customer's own AI — that need is already met.
- **Same brain (account 863), not a separate account** — one knowledge base serves prospects on the landing page and customers in the dashboard.
- **Attribution ≠ context injection, and they're separable.** Attribution (the conversation knows it's JPH asking) is small — the dashboard already knows who's logged in — and is ALL a support inbox needs. Context injection (the AI knows their toll-free is pending) is the big build and only makes answers smarter.
- **Support conversations must NOT create leads.** `/api/widget/[id]/chat` extracts email/phone → creates a contact → fires hot-lead alerts. Right for the landing page, wrong in the dashboard: a paying customer asking for help would land in Leads and could trigger a hot-lead alert. Dashboard widget declares itself support; chat route skips contact creation + alerts, keeps knowledge capture. Same problem intent-triage solved for email.
- **The support inbox is an ADMIN feature, not a dashboard tab** — every customer sees the same sidebar, so a "Customer Support" tab would appear for JPH with nothing in it. Belongs on `/admin/dashboard`. General rule worth reusing: platform-operator features (support inbox, unit economics, churn) don't belong in the product dashboard.
- **Build order:** widget + attribution + support marking first (leads stay clean, data right from day one, unanswered questions already flow to the existing queue with no new UI) → admin support view once there's traffic worth looking at → context injection last.
- **Content seeding:** the chatbot KB is ~780 chars written for prospects; customers ask "how do I", almost no overlap. Draft ~15 support answers FROM THIS FILE (Outlook/Gmail connect, SMS provisioning + why TFV takes 3-5 days, AI Settings, booking, billing) for founder review rather than writing from scratch.

**🎙️ NEXT UP — EVALUATE ELEVENLABS vs VAPI (founder's next work item, 2026-07-28).** Two separate questions: (a) use an ElevenLabs *voice* inside Vapi — a config change, not a rebuild, but note the codebase moved OFF 11labs "rachel" on 07-06 because the voice id was invalid (`VAPI_VOICE_PROVIDER` / `VAPI_VOICE_ID` env), so verify the id this time; (b) replace Vapi entirely with ElevenLabs Conversational AI. Compare on what BizzyBot actually depends on, not on voice quality alone: mid-call function calling (calendar booking), Twilio BYON number import, an end-of-call webhook carrying transcript + summary + cost (the voice half of the unanswered-questions queue, the document follow-up and the Unit Economics actuals ALL read it), per-customer assistant provisioning, recording URLs, and per-minute cost vs Vapi's ~$0.09. Migration is NOT trivial despite the "swap a provider" framing — `lib/vapi.js`, `lib/voice-gap-scan.js`, `lib/voice-document-followup.js`, `app/api/vapi/*` (provision/webhook/stats/call-settings/recording), `lib/voice-routing.js`, plus `vapi_call_logs` and the `vapi_assistant_id`/`vapi_voice_url` columns all assume Vapi's shapes. Full note in memory ([[vapi-voice-integration]]).

**🎨 TODO — per-customer widget logo (raised 2026-07-28, deliberately deferred).** Every customer's chat widget currently shows the same generic robot icon, so it reads as a bolted-on third-party tool rather than their business. Founder correctly ruled out hard-coding BizzyBot's logo — one script serves every customer's site. The right fix is a per-customer logo, BUT a good version needs file upload → object storage, which is the same gap blocking **Documents Phase B**; build them together or you build it twice. A URL-only field works for BizzyBot and is useless for a plumber whose logo is on their laptop. Cheap interim if it ever matters sooner: the business's initial in a circle, the same treatment the dashboard already uses for lead avatars — per-customer, no storage, and at 60px it reads better than a shrunken logo anyway. Note for the record: the widget NEVER showed a brand logo; it was the `🤖` emoji, which on Windows renders as a chunky robot and read as BizzyBot's own mark.

**🧹 SAFE TO DELETE (verified unreferenced 2026-07-28):** `app/components/AnalyticsDashboard.js` (573 lines) — zero imports, zero links anywhere in app/, components/, lib/ or middleware. Removing it empties `app/components/` entirely, which is the point: the real components live in `/components`, and `app/components/` is pre-pivot residue. Its two siblings (`ChatWidget.js`, and `app/sms-dashboard/page.js`) were deleted in 0e173cd; this one was left only because it was outside what had been agreed at the time.
...
**📌 TRADEMARK — "BizzyBot" name is CLEAR (checked 2026-07-22).** The one conflicting USPTO application (BIZZYBOT, serial 98377405, filed Jan 2024, Class 042 — our exact category) is **DEAD**: applicant filed express abandonment, abandoned after publication Apr 3 2025. No active federal mark blocks us. **ACTION ITEM (not urgent; do before scaling marketing hard): file our OWN trademark** — owner "Bizzy Bot Ai LLC", primary Class 042 (SaaS), use-based (live site + real customer). Recommend attorney / flat-fee service WITH a clearance search (crowded name + bizzybotnj.com is a common-law user in a different lane). ~$350 gov fee/class + $0 DIY to ~$500–1500 attorney. Neither founder nor Claude is a lawyer — get counsel. Sitewide Organization schema already added (`app/layout.js`) for entity disambiguation; `sameAs` profile links deferred until founder owns real brand profiles (LinkedIn/G2/Capterra).
...
### Session — 2026-07-27/28 (SMS compliance · landing motion + widget · call recordings · "keep your number" verified)

- **Toll-free SMS compliance gap found and closed.** Live traffic carried no business name and no opt-out instructions, while the approved TFV `ProductionMessageSample` in `lib/tollfree-verification.js` promised "Reply STOP to opt out, HELP for help." Twilio's TFV guide requires both "at regular intervals throughout the conversation", and requires samples and real traffic to "tell a consistent story" — that mismatch is what an audit compares. Fixed in `lib/sms-compliance.js`: first message per conversation, every 10th after, always on business-initiated follow-ups. Hard-coded in the send path (founder's call — a customer would remove it and BizzyBot holds the verification). **Review caught that the idempotency guard was itself the bypass**: it matched `/reply stop/i` against the AI's finished text, which a customer shapes via custom instructions, so telling the AI to sign off "reply STOP" would have silently stripped the business-name disclosure forever. Now matches only its own footer. Also em dash → hyphen: one non-GSM-7 character switches the whole SMS to UCS-2, cutting a segment from 153 chars to 67 and roughly doubling cost on every footer-bearing message.
- **Landing page motion.** `components/marketing/Reveal.js` — scroll reveals on seven sections plus a hero dashboard where leads keep arriving. **Fails VISIBLE by design**: content is NOT hidden in the server-rendered HTML (verified: 59KB with every section present, zero `opacity-0`), because this site lost seven weeks of indexing to content Google could not see. Declined Framer Motion — ~40kb of JS on the most SEO-critical page to do what a CSS transition does better. Founder feedback fixed two things: the hero animation fired on page load while the dashboard sat below the fold (now starts on scroll-into-view and loops), and the "AI receptionist" badge duplicated the subhead directly beneath it (cut).
- **Chat widget on landing + all marketing pages** (`components/marketing/DemoWidget.js`, mounted in `MarketingFooter`). BizzyBot answering for BizzyBot — every prospect question is both a demo and a lead, and anything it cannot answer lands in the queue as direct feedback on our own marketing. Fixed three things while there: the widget greeted visitors with the LEGAL entity name; AI replies rendered markdown literally as one crushed paragraph (now a tiny escaped-first markdown subset with `white-space: pre-wrap`); and the 🤖 emoji became the Lucide `Bot` SVG the dashboard already uses. Deliberately NOT the BizzyBot logo — the widget script is served to every customer's site and would brand a plumber's page as BizzyBot.
- **Call recordings.** `recordingEnabled: true` had been set in `lib/vapi.js` since launch, so Vapi held audio for every call ever taken and nothing stored the URL. Now captured, announced in the greeting on every call, and playable. **The playback bug is the one worth remembering** — see Code Gotchas: the stored URL is a private bucket that 400s, the playable form expires in 30 minutes, and storing THAT would have worked on the day it shipped and been broken by morning.
- **Recording consent: built the clever version, then deleted it.** First implemented per-caller area-code detection (14 all-party states, fail-closed) so no announcement would be needed for one-party callers. Founder reversed it — announce on all calls, keep all recordings — and was right: area code is a proxy for location not a location, Vapi cannot vary greetings per call without the `assistant-request` webhook anyway, and the conditional version was more code, more failure modes and LESS legal certainty. Module deleted rather than left dormant; unused compliance logic invites someone to wire it up later assuming it was vetted.
- **Caller names fixed** (founder-reported): leads stayed "Unknown" while the call summary beside them named the person. The transcript scan extracted it but only wrote it to `knowledge_gaps`.
- **✅ "Keep your existing number" verified live** — see the pinned entry in NEXT SESSION TODO. Materially changes what can be promised on a sales call.
- **Prospecting research** (founder was building a call list): ServiceTitan's Google "Book online" button is **Reserve with Google** (`rwg_token` in the URL), not something ServiceTitan built. Priced $245–500/tech/month + $5K–50K setup, so a business running it is a DISQUALIFIER, not a prospect. Also scoped the public booking page (`docs/superpowers/specs/2026-07-27-public-booking-page-scope.md`). Useful prospecting habit that came out of it: **look for the promise a site makes that its plumbing does not keep** — McKinney Heating's prominent "SCHEDULE NOW" button links to `/contact-us/`, a plain form. Verified from two pages before treating it as a call opener.
- **Late-session cleanup, all triggered by the founder asking "is this even used?"** — a question that paid for itself several times over:
  - **THE PUBLIC DEMO WAS BROKEN.** `/demo` was in publicRoutes and was the landing page's "See it in action" CTA (nav, mobile menu, hero, footer), but POSTed to `/api/chat`, which requires a Clerk session. Verified against production: page 200, chat call **401**. Every logged-out prospect who clicked through and typed got nothing. **`/demo` is now deleted entirely** — it was a second, worse copy of the chat bubble that had just been added to every marketing page. Seven references cleaned up including the contact-form auto-reply, which emailed people a link to it. The hero's secondary CTA is now **"Talk to it now"** and opens the bubble, polling briefly first because the widget is `lazyOnload` and a fast click can beat it.
  - **Real-estate leftovers purged.** `app/api/chat/route.js`'s catch branch set an unconfigured customer's system prompt to *"You are a professional AI assistant for a real estate business"*, and carried its OWN copy of the hot-lead keywords ("cash buyer", "schedule showing") which never received the 07-19 de-fang and scored a single match at 70, above the hot threshold of 60. Now industry-neutral and using the shared `lib/hot-lead-keywords.js` with its cap of 45. Also fixed: sign-in/sign-up page copy ("Access your AI real estate assistant") and two placeholders.
  - **590 lines of dead code deleted** (`app/components/ChatWidget.js`, `app/sms-dashboard/page.js`) — both verified unreferenced. The structural tell was two folders doing the same job: `app/components/` alongside `/components/` is a half-finished migration.
- **Process note:** every bug this session was found by the founder testing live, none by review. Reviews caught structural defects (races, leaks, bypasses); only real traffic caught assumptions about the world — which placeholder word this model writes, that `email_settings` has its own `id`, that a presigned URL expires.
...
- **Deep-research pass** on market demand + positioning + AI-search ranking (full writeup: `docs/marketing/2026-07-21-seo-vertical-strategy.md`). Demand order confirmed by data: **Trades (HVAC/plumbing/contractors) #1** (HVAC lose ~$45.6k/yr to missed calls, 74% of calls unanswered), Salons #2, Real Estate #3 (huge ROI but most saturated keywords).
- **Positioning (founder-approved): multi-channel + price** — "every channel Podium answers, one-tenth the price, built for the solo operator." Beats voice-only bots (Rosie/Goodcall/etc, phone-only) on channels; beats Podium ($399+$99 AI)/Birdeye/GHL on price. **Deliberately NOT leading with "GoHighLevel alternative"** — high volume but attracts agencies (wrong buyer); Podium-alternative is the right comparison target.
- **GEO (ranking in ChatGPT/Perplexity) = the priority lever.** Key insight: AI recommends brands that appear in third-party sources (G2, Capterra, Reddit, "best AI receptionist" listicles) — your own site isn't enough. On-page GEO baked into every new page: 40–60 word direct answer up top, stats WITH cited sources, FAQPage + Article + SoftwareApplication + BreadcrumbList schema, semantic URLs. **Off-page GEO is founder work (~50% of the win): get on G2/Capterra/Trustpilot/Product Hunt, a few Reddit mentions, email listicle authors, get JPH to link + review.**
- **Built (7 files):** `components/marketing/MarketingChrome.js` (shared nav/footer/CTA/JsonLd, server components, JSON-LD `<`-escaped) + `components/marketing/VerticalLanding.js` (data-driven GEO template) → 3 industry pages (`/ai-receptionist-for-contractors` flagship, `/ai-receptionist-for-salons`, `/ai-receptionist-for-real-estate`), `/podium-alternative` (comparison table), `/blog/ai-receptionist-cost` (buyer-intent pricing guide). All server-rendered w/ own metadata. Updated `app/sitemap.js` + **`middleware.js` publicRoutes** (critical — else crawlers get redirected to sign-in). Build clean.
- **Note:** site still not indexed (`site:bizzybotai.com` empty) — expected, only unblocked 07-20; re-check weekly. Homepage industries section could link to these pages (internal-link win) — not done, recommended next.
...
- **Text/email/chat channels** — `lib/ai-service.js` gains a "Step 1.3" gate right after customer resolution: if `hasActiveAccess()` is false, `generateAIResponse()` returns `{ success: false, trialExpired: true }` (short-circuits BEFORE the OpenAI call, so no cost) — deliberately not the softer `success: true` placeholder the usage-pool cap uses.
- **⚠️ Double-check pass (same session) caught a real gap:** the initial assumption that "every channel skips on `!success`" was WRONG. Most lead-facing channels treat `success:false` as a technical error and send a CANNED FALLBACK ("I'm experiencing technical difficulties" / "Thanks, we'll get back to you shortly") — so trial-expired leads got a misleading message, not silence. Web chat was worse: `/api/chat` throws → a legacy fallback OpenAI call (owner-only test tool, left as-is). Email triage's `conservative_reply` branch replies via a template that never calls `generateAIResponse`, so the Step 1.3 gate never ran. **Fixes:** (1) new `isAiSilencedForClerkUser(clerkUserId)` in `lib/trial-access.js` (fails OPEN on unknown owner / DB error). (2) SMS webhook, Facebook DM, Instagram DM, widget chat now check the `trialExpired` flag and send NOTHING (FB/IG *comment* paths + email webhook were already silent). (3) Outlook monitor gates per-account before triage; gmail `respondToEmail` gates at entry (gmail follow-up loop was already covered by the internal gate). (4) Widget was least-broken — its auth route already gated on `hasActiveAccess` and the client self-removes on 402; deduped that route's copy of `hasActiveAccess` into the shared module. (5) Fixed a metric mismatch: `getVoiceMinutesThisMonth` counted by `created_at` but the `/voice` dashboard counts by `started_at` — aligned to `started_at` so enforcement matches the number the customer sees.
