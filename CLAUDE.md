...
**📌 PINNED BUG (found 2026-07-21) — TRIAL ENFORCEMENT DISABLED THE FOUNDER'S OWN DEMO ACCOUNT (863).** Account 863 has NO Stripe subscription and is >14 days old, so `hasActiveAccess(863)` = false → the new gate switched its AI voice OFF (calls forward to cell / hang up instead of the AI answering). This is *correct* for a real expired customer but WRONG for 863, which is the founder's demo account used to show prospects. **Fix (not yet done, ~2 min): add a comp/allowlist in `lib/trial-access.js`** so demo/admin accounts (customer 863 + `ADMIN_CLERK_ID_1`) always return `hasActiveAccess: true` regardless of subscription/trial; real customers stay enforced. NOTE: this ALSO silences 863 on SMS/email/chat/web (not just voice) — the whole demo account is currently gated off. SEPARATE non-bug: founder's "enter your PIN" prompt when calling 866 = calling the toll-free FROM the same cell it forwards to → carrier voicemail asks for the voicemail password; predates all updates, unrelated — always test voice from a DIFFERENT phone. (Silver lining: this confirms the enforcement actually works.)
