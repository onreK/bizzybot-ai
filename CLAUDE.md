...
**📌 Documents Phase A — one retest from done.** Tasks 1-3 built+verified (doc targeting w/ 'when' field · voice email delivery · post-call pipeline; see `docs/superpowers/plans/2026-07-18-documents-phase-a.md`). First real call exposed a phantom-dot email bug (extraction emailed kerno.junk@ instead of kernojunk@) — FIXED (prompt + extraction hardening, assistant re-synced, verified live on Vapi). **Task 4 retest blocked overnight**: founder's calls to (866) 944-5685 stopped reaching Twilio entirely (no call records — carrier-side toll-free routing, same self-healing pattern as 07-12; app/Twilio/Vapi all verified healthy). **DO: retest call from any phone EXCEPT the founder's cell** (ask for pricing guide, spell kernojunk@gmail.com, expect exact letter-by-letter read-back, email to the RIGHT address — delivery works now that Resend is fixed) → then mark Task 4, move feature to Completed, decide fate of test doc "BizzyBot Pricing Guide" on 863's voice channel. ~~CARRIER ISSUE ISOLATED 07-19: ONLY calls from the founder's cell die~~ **⚠️ SUPERSEDED 07-19 evening: voice failure is BROADER than founder's carrier** — ring-then-hangup now reproduced from THREE phones: founder's cell (T-Mobile), a second T-Mobile phone, AND an Xfinity Mobile phone (Verizon network) → two carrier networks failing kills the "founder's carrier routing" theory. **DIAGNOSED 07-19 evening (systematic-debugging, evidence-complete):** the failing calls produce ZERO Twilio call records; account active, number in-use, voice_url correct, no new Twilio alerts; a Twilio-internal API call placed DURING the outage window connected 35s + AI answered (SID CA167ea19f33a83a37fcdaedf0075854a6, ~00:0x UTC 07-20; morning twin SID from +18886014185 07:08 UTC 07-19 also 35s). Conclusion: **toll-free routing failure in the public phone network upstream of Twilio, affecting ≥2 originating networks — Twilio/RespOrg-side, NOT BizzyBot code, NOT the founder's carrier.** Third occurrence in a week (07-12 degraded window, 07-18/19 overnight, 07-19 evening) → escalating pattern. **CONTROL TEST 07-20 01:57 UTC nails it to THIS NUMBER: founder's same cell (858-900-4220) called (888) 601-4185 — same account, also toll-free — completed 5s and logged, while its calls to (866) 944-5685 leave zero records.** Number-specific registry/routing fault. ACTION: founder files urgent Twilio support ticket (draft provided in session); nothing app-side can fix this. Last resort if Twilio can't repair routing: replace the number (painful — TFV redo 3-5 days + number is on marketing materials; push Twilio to FIX first). Parked decisions: "text you the link" for mobile callers (recommended next); Phase B storage/attachments + Phase C receiving deferred. Also same-day: TFV self-service shipped (branded /sms-optin+/sms-terms, legal-vs-brand DBA, /api/admin/resubmit-tfv + /api/admin/sync-vapi ops routes, status card); JPH resubmission parked on their NEW LLC (old one in bad standing).
...
- **⚠️ Voice regression report (same evening)**: (866) 944-5685 ring-then-hangup from 3 phones across 2 carrier networks (2× T-Mobile + 1× Xfinity/Verizon) — supersedes the "founder's carrier only" isolation from this morning. Investigation in progress.
